import { Button, StyleSheet, Text, TextInput, TouchableOpacity, View, Image, FlatList, ScrollView } from 'react-native';

export default function Popular(){
    return(
        <View style={{backgroundColor:"#FAFAFD", borderWidth:0, borderRadius:15}}>
        ...
        renderItem={({item}) => (
            <View style={{display: 'flex', flexDirection: 'row', gap: 10, marginLeft: 0, backgroundColor:"white", height:90, width:300, padding:30, marginLeft:10, marginBottom:10, borderRadius:25}}>
                <Image source={item.image} style={{width: 50, height: 50, borderRadius: 14, backgroundColor:"white", marginTop:-16, marginLeft:-24}} />
                    ...
                    <View style={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop:-15, gap:5}}>
                    <Text style={styles.cardHeaderText}>{item.title}</Text>
                    <Text style={styles.bottomText}>{item.company}</Text>
                    </View>
                    <View style={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop:-15, marginLeft:0, alignItems:""}}>
